...
class Model:
    ...
    def update_K_values(self):
        ...
        print(self.CompoundData[c].p_vap(self.T[:]) for c in self.components)
        p_tot = sum(self.CompoundData[c].p_vap(self.T[:])*self.x[c] for c in self.components)
        p_tot[p_tot==0]=1
        print(p_tot)
        for c in self.components:
            self.K[c][:] = self.CompoundData[c].K_func(self.T[:],p_tot)
    ...
    def solve_self(self):
        ...
        self.T_feed = 300#self.bubble_T_feed(stage=self.feed_stage)   ### XXX need to update this to take in the array
     
        for i in self.stages:
            self.T[i] = self.T_feed   ### initialize T
